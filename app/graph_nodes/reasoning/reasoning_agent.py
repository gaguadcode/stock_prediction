...
from ollama import Ollama  # Ensure Ollama is installed: `pip install ollama`
...
class ReasoningNode:
    ...
    def __init__(self):
        ...
        self.agent = Ollama(model="deepseek")  # Instantiates the Ollama model
    ...
    def generate_reasoning(self, input_data: Union[UserInputString, ResearchOutput]) -> ReasoningOutput:
        ...
        try:
            ...
            logger.debug(f"Response from DeepSeek: {response}")

            # Parse response and return structured reasoning
            return ReasoningOutput(
                user_input=UserInputString(user_input=input_data.user_input),
